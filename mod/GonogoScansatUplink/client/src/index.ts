...
// @ksp-gonogo/core's global component registry:
//
//   - `uplink.ts` → defineUplinkClient({ id: "scansat", ... }) declares this
//     client's identity; every registration below stamps the returned
//     SCANSAT handle as `owner`, so the widget picker's mod search tags
//     derive "scansat" automatically.
//
...
//     `MapPoiLayer`, gaining the uniform "Set as Target" action, replacing
//     the old `map-view.overlay` augment + its bespoke bearing/distance
//     panel.
//   - `FootprintOverlay` → registerAugment({ id: "scansat-footprint-overlay",
//     ... }) so it fills the same `map-view.overlay` slot with scanning-
//     vessel ground-track footprints, replacing the old MapView-internal
...
//     coverage readout, replacing the old MapView-internal
//     `CoveragePanelView`/`CoverageRow`.
...
//     colormap surface (altimetry or biome) modulated per-tile by the
//     coverage paint-gate, replacing the old MapView-internal
//     `useBiomeCanvas`/`useHeightCanvas`.
//   - `FogReveal/useScanSatFogSync` → registerFogRevealSource(...) once per
//     scan type ("scansat:AltimetryLoRes" etc.) so MapView's coverage
//     paint-gate knows this Uplink contributes fog reveal, even before
//     anything calls useScanSatFogSync itself.
//
...
// Uplink's own canonical copy. `packages/core`/`packages/data` still carry a
// duplicate for `packages/components`'s MapView, which hasn't migrated off
// it yet; that duplicate is slated for deletion once MapView's augment
// migration lands. The Minimap here (`Scanning/Minimap.tsx`) has its own
// mod-local coverage gate (`FogReveal/useScanCoverageGate.ts`) and paints
// through `TerrainBase/paintTile.ts`, same as BiomeBase: it no longer
// borrows MapView's canvas hooks via @ksp-gonogo/components at all.
