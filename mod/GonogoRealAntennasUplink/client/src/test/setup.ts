 ...
 * registerComponent at module load, because RealAntennas ships no widget
 * (see index.ts). The hook is still needed, since the hydration test proves
 * the relocated unit registrations by DECODING a frame through the real
 * pipeline rather than by reading a registry, and that goes through the
 * facade.
 */
installTestHost({
  useTelemetry,
