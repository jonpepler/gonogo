...
describe("parseKosData", () => {
  ...
  it("parses through ANSI cursor-position escapes that wrap kOS output", () => {
    /**
     * Real-world capture: kOS's GUI repaint emits the screen contents with
     * `ESC [ row ; col H` injected at every PTY-width line wrap. Long PRINT
     * output gets split across rows, breaking literal marker matches like
     * `[/KOSDA<ESC[22;1H>TA]`. The parser must ignore these escapes when
     * locating the block AND when reading the body.
     */
    const wrapped =
    ...
    expect(parseKosData(chunk)).toMatchObject({
       ...
       * that's fine for shipmap (it parses parts as JSON afterwards). What
       * matters is that the marker pair was found at all.
       */
