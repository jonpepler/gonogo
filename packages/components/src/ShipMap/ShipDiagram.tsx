...
interface Props {
  ...
  throttle?: number;
  /** Per-part resource meters (self-contribution), keyed by
...
export function ShipDiagram({
  ...
  /**
   * The hovered part's contributed rows: rendered here through the real
   * ui-kit `<Meter>`, the SAME component ShipSystems and every other
   * meter-bearing widget uses, so a contributed reading and a built-in one
   * look like one system rather than two. `ShipDiagramSvg`'s compact
   * in-body bars read the identical `partMeters` map (passed straight
   * through above); this is the OTHER rendering of that one aggregated
   * list, not a second data path.
   */
