 ...
 * redesign surfaces `panelBadges`, DISTINCT from the widget-authored
 * per-row `crew-status.badges` AugmentSlot the CrewStatus base widget
 * itself declares for `CrewSurvivalBadgeAugment` in `index.tsx`: same
 * slot NAME, two different registries, `augments.ts`'s SlotRegistry for
 * the per-row one and `contributions.ts`'s ContributionRegistry for this
 * one). Fed by the SAME `CREW_SURVIVAL` Processor `index.tsx`'s per-row
 * augment reads: one per-frame evaluation, two consumers.
 *
 ...
 * (`tone === "nogo"`, the same threshold the per-row badge in
 * `index.tsx`'s `warningFor` uses for an imminent death clock or a rule
 * past its critical fraction): a merely-elevated ("warn") crew is already
 * flagged per-kerbal by the `.survival` meter's own colour, so the header
 * stays quiet for that case and reserves itself for what actually
 * threatens someone. Returns null when nobody is critical, so a nominal
