 ...
 * now" and, historically, answered it with a telemetry read:
 * `useTelemetry(`${requires}.available`)`. That made the gate a SPINE read,
 * which stranded `AugmentSlot` in `@ksp-gonogo/core` even though everything
 * else about the augment model is spine-free.
 *
 * The gate now reads a ui-kit-OWNED store instead, exactly the shape
 * `DelayRailContext` uses: ui-kit defines the store + context; the APP
 * injects real `<domain>.available` presence into it from telemetry (see the
 * app's augment-availability feeder). ui-kit gains NO spine dependency, and
 * with no provider mounted the gate falls back to the same answer the old
 * telemetry read gave with no `TelemetryProvider`: a Domain nothing has
