...
function ScienceDataComponent({
  ...
  // whose model is not mits leaves `dataAmount` null on every entry (a
  // provider storing megabytes says so through `valueModel`), and summing
  // those to a
...
registerComponent<ScienceDataConfig>({
  ...
  // controls arrive through the augment slot noted above, dispatched from
  // within the slot rather than through this widget's own action list).
  actions: [],
