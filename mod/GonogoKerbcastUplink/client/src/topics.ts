 ...
 * contract slice (GonogoKerbcastUplink.Contract), so it is now bare-registered
 * here too, same shape as `kerbcast.available`, just with a real generated
 * payload type behind it instead of `boolean`.
 *
 ...
 * augmentation above is in-program and resolves the Topic to its real
 ...
 * cannot see this augmenting module (2026-07-20). Kept inline (type-only,
 * erased at runtime) rather than in a `.test-d.ts`: the client's build
 * tsconfig does not exclude `*.test-d.ts`, so a separate file would be
 * emitted into `dist`.
 */
type Equal<A, B> =
