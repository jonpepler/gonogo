 ...
 *
 * 1. **toggle -> absolute.** Every legacy `f.<x>` action is a pure "flip
 ...
 *    Both are real `LEGACY_COMMAND_HOMES` entries below now, not
 *    `KNOWN_COMMAND_GAPS`.
 ...
 * see the project's own legacy maneuver-node arg-order finding,
...
const LEGACY_COMMAND_HOMES: Readonly<Record<string, CommandHome>> = {
  ...
  "f.stage": {
    ...
    // entirely (`object? _`), matching the legacy void fire-and-forget.
  ...
  // `crewSemis = Array.from(selectedCrew).join(";")`: the legacy split action
...
export const KNOWN_COMMAND_GAPS: ReadonlySet<string> = new Set([
  ...
  // LEGACY_COMMAND_HOMES entries; career.status.* already streams every
  // id these key on.
  ...
  // see the ksp.*/tar.switchVessel LEGACY_COMMAND_HOMES entries.
]);

/**
 * Splits a legacy action string into its key and bracketed args,
...
export function hasCommandHome(dataSourceId: string, action: string): boolean {
  if (dataSourceId !== "data") return false;
  const { key } = parseLegacyAction(action);
  return key in LEGACY_COMMAND_HOMES || ACTION_GROUP_KEY_PATTERN.test(key);
}
