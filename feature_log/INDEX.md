...
- 2026-05-16: [Ship Map Phase 2: harness, snapshots, sizing, freeze fix, orientation, fuel-line pipes, part-state overlays](2026-05-16-ship-map-phase-2.md), `14c4756`..`2018b94` (+ fork `c0cc3bd`..`2f852ec`): ⏳ pending (SVG-render harness + snapshot tests; useTopology cascade freeze fix; heat-tint overlay; bounds.size axis fix; cargo-bay classification gate; fork-emitted `up` vector + client rotation with edge-on guard; wheels as circles; decoupler orientation from body proportions; fuel-line pipe + flow chevrons + fuelLineTarget; bounds.center for radial-mount parts; part-state overlays, engine flame, parachute canopy, deploy chevron, gear stand, cargo bay open; new inspect_fixture.py helper. Needs a fresh KSP capture against the rebuilt DLL to verify bounds.center fix + a live partState demo to fully promote)
