 ...
 * REAL browser on all three engines, that ALL THREE first-party Uplinks
 * (scansat + kos + kerbcast):
 *
 ...
 *     ONE registry (`scanning` + `kos-terminal` + `camera-feed` all appear);
 ...
 *     `scanning` widget before the flagged navigation; because `main.tsx` only
 *     calls `renderApp()` AFTER `loadEnabledUplinks` resolves (registerScansatAndRender
 ...
 *  5. flag OFF, all three widgets are still present via the bundled static imports
 *     — the fallback the design forbids removing until the loaded path is
 *     triple-engine green (§6 / R9) — and NEITHER loader bundle is requested.
 *
...
test.describe("Uplink loader (dual-path)", () => {
  test("scansat + kos + kerbcast load via the runtime loader into the singleton registry", async ({
    page,
  }) => {
    // Establish the origin, then seed consent + the dashboard/Settings-UI
    // extras so the flagged reload reaches import() and the follow-on render
    ...
    // the flagged navigation. `main.tsx`'s `registerScansatAndRender` only
  ...
  test("flag OFF keeps scansat + kos + kerbcast via the bundled static imports (fallback intact)", async ({
    page,
  }) => {
    ...
    page.on("request", (r) => {
      if (
        ...
        r.url().includes("/uplinks/kerbcast.client.js")
      ) {
    ...
    // All three widgets are still present — via the bundled static imports,
    // not the loader.
    await expect
      .poll(
        async () => {
