...
describe("rowTones", () => {
  ...
  it("flags a single critical kerbal by name, alert tone", () => {
    expect(
      rowTones(
        survival([
    ...
    ).toEqual([{ crewName: "Jebediah Kerman", tone: "alert" }]);
  });
  ...
  it("flags every critical kerbal, never the ones still nominal", () => {
    expect(
      ...
      { crewName: "Jebediah Kerman", tone: "alert" },
      { crewName: "Bill Kerman", tone: "alert" },
    ]);
  });
});
