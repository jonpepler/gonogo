 ...
 *
 * Call `registerStockBodies()` once at app startup to populate the registry with
 * every stock KSP celestial body. The IDs match the body name the telemetry stream
 * reports, so a reading's body name looks up directly through `getBody`.
 *
 * Radii sourced from the KSP wiki (accurate to stock KSP 1.x). An external package
 * (a mod, a planet pack) calls `registerBody()` afterwards to add or override
 * entries, which is why last write wins there.
 */
...
export function registerStockBodies(baseUrl = "bodies"): void {
  ...
  registerBody({
    ...
    parent: "Sun",
    texture: tex("Kerbin"),
    // Texture prime meridian is offset 90° east of the stream's reported frame.
    ...
    // KSC sits at roughly (-0.10°, -74.56°) in the raw reported frame. Seed
