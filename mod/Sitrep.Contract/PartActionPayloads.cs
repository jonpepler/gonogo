...
{
    /// <summary>
    /// The vessel this part is on: the RAW <c>Vessel.id</c> guid, the same
    /// identity <c>fleet.&lt;guid&gt;</c> and <see cref="VesselIdentity.VesselId"/>
    /// key by. Carried for the reason <c>Sitrep.Host.VesselAttribution</c>
    /// exists: this is an active-vessel read on a topic whose name carries a
    /// PART id, not a vessel guid, and it is <see cref="DelayRole.Delayed"/>, so
    /// a delivered payload is light-time old while the client's notion of the
    /// active vessel is whatever it holds now. Without this there is no in-band
    /// way to see a vessel-switch boundary. <c>null</c> when there is no active
    /// vessel or its identity carries no id.
    /// </summary>
    [SitrepUnit(Units.Id)]
