...
namespace Sitrep.Host
{
    ...
    ///     "currentTemp"|null, "skinTemp", "category", "categoryOrdinal"|null,
    ///     "modules": [ "ModuleEngines", ... ],
    ///     "isRobotics", "isPowerRelated", "fuelLineTargetId"|null,
    ///     "resources": { "<name>": { "amount", "maxAmount", "flow"|null, "nominalFlow"|null }, ... },
    ///     "moduleStates": [ { "type", "state", "tracking"|null, "flameout"|null }, ... ] }, ...
    /// ]
    /// </code>
    ///
    ...
    public static class VesselPartsViewProvider
    {
        ...
        private static VesselPart MapPart(IDictionary<string, object?> raw)
        {
            return new VesselPart
            {
                ...
                CategoryOrdinal = (KspPartCategory?)GetInt(raw, "categoryOrdinal"),
        ...
        {
            var result = new Dictionary<string, PartResourceFlow>();
            if (raw.TryGetValue("resources", out var rawResources) && rawResources is IDictionary<string, object?> resources)
            {
                foreach (var kvp in resources)
                {
                    if (kvp.Value is IDictionary<string, object?> row)
                    {
                        result[kvp.Key] = new PartResourceFlow
                        {
                            Amount = GetDouble(row, "amount") ?? 0.0,
                            MaxAmount = GetDouble(row, "maxAmount") ?? 0.0,
                            Flow = GetDouble(row, "flow"),
                            NominalFlow = GetDouble(row, "nominalFlow"),
                        };
                    }
                }
            }
        ...
        private static List<PartModuleState> MapModuleStates(IDictionary<string, object?> raw)
        {
            var result = new List<PartModuleState>();
            if (raw.TryGetValue("moduleStates", out var rawStates) && rawStates is IEnumerable<object?> list)
            {
                foreach (var entry in list)
                {
                    if (entry is IDictionary<string, object?> row)
                    {
                        result.Add(new PartModuleState
                        {
                            Type = GetString(row, "type") ?? "",
                            State = GetString(row, "state") ?? "",
                            Tracking = GetBool(row, "tracking"),
                            Flameout = GetBool(row, "flameout"),
                        });
                    }
                }
            }
        ...
        private static Dictionary<string, object?> ToWire(VesselPart part) => new Dictionary<string, object?>
        {
            ...
            ["categoryOrdinal"] = part.CategoryOrdinal.HasValue ? (int)part.CategoryOrdinal.Value : (int?)null,
        ...
        private static Dictionary<string, object?> ToWire(PartModuleState module) => new Dictionary<string, object?>
        {
            ["type"] = module.Type,
            ["state"] = module.State,
            ["tracking"] = module.Tracking,
            ["flameout"] = module.Flameout,
        };
