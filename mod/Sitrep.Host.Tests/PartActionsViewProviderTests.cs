...
namespace Sitrep.Host.Tests
{
    ...
    public class PartActionsViewProviderTests
    {
        ...
        [Fact]
        public void WireCarriesEveryDeclaredFieldIncludingTheVesselSubject()
        {
            var actuator = new FakePartActionActuator();
            ...
            // The vessel subject: a per-part topic name carries no guid, so without
            // this a light-time-old payload could not be told from the current
            // vessel's after a switch (see PartActions.VesselId).
            Assert.Equal("guid-9", wire["vesselId"]);
        ...
        public void CacheLetsThroughTheSameListOnADifferentVessel()
        {
            ...
            // differs in the field that says which craft it describes.
            var actuator = new FakePartActionActuator();
            actuator.ActionsByPartId["1"] = new List<PartActionEntry> { Entry("Deploy") };
            var cache = new PartActionsPublicationCache();
