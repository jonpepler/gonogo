...
namespace Gonogo.KSP.Tests.Comms
{
    ...
    public class CommNetBackendSharedShapeTests
    {
        ...
        public void TheSharedShapeIsImplementedOnceInTheContract()
        {
            ...
            {
                var method = typeof(CommNetBackend).GetMethod(name, BindingFlags.Public | BindingFlags.Instance);
                Assert.True(method != null, "CommNetBackend no longer implements " + name);
                Assert.True(
        ...
        public void TheJudgementStaysWithTheBackend()
        {
            foreach (var name in OwnJudgement)
            {
                ...
                Assert.True(method != null, "CommNetBackend no longer implements " + name);
                Assert.True(
                    method!.DeclaringType == typeof(CommNetBackend),
                    name + " is answered by " + method.DeclaringType?.Name + " rather than by "
                    + "CommNetBackend: routing, reach and occlusion are the rules a backend OWNS, and "
        ...
        /// <summary>
        /// The downcast is gone, not merely bypassed. Naming a command centre
        /// used to live on this concrete class, so the core comms registration
        /// reached it through <c>backend is CommNetBackend</c> and every other
        /// backend got an all-null payload forever. A public
        /// <c>CommandCentre</c> member back here is that path reopening.
        /// </summary>
        [Fact]
        public void NamingACentreIsNoLongerThisBackendsPrivilege()
        {
            var concreteOnly = typeof(CommNetBackend)
                .GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly)
                .Where(m => m.Name.Contains("CommandCentre"))
                .ToArray();

            Assert.True(
                concreteOnly.Length == 0,
                "CommNetBackend declares " + string.Join(", ", concreteOnly.Select(m => m.Name))
                + ": naming a centre is a universal question and belongs on the seam, reachable "
                + "through ICommsBackend.ControlPathTerminus");
        }
    }
}
