...
function warningFor(
  ...
  if (kerbal.deathClockSec !== null) {
     ...
     * through `formatDuration`, the same composite ladder the
     * delay/countdown strips use, never `speakQuantity(value("s", ...))`.
     * Routing a duration through the scalar `time` unit-kind is how this
     * used to render "~4M TO FATAL", an ambiguous "M" indistinguishable
     * from metres once the Badge's `text-transform: uppercase` got hold
     * of it.
     */
    return {
