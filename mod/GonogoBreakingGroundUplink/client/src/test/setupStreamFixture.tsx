...
export function setupStreamFixture(opts: StreamFixtureOptions): StreamFixture {
   ...
   * namespace: tell the store so a 3+-segment dynamic topic
   * (fleet.<guid>.delay) is subscribed/sampled whole, not mis-split into a
   * `<parent>.<field>` the wire never publishes. Exact (non-`.`-terminated)
   * carried topics are unaffected.
   */
   ...
   * shared helper (including the probe/visual-gate render harness) gets
   * them for free instead of each widget's own test file registering them
   * by hand (FuelStatus/index.test.tsx used to be the only place that did).
   */
