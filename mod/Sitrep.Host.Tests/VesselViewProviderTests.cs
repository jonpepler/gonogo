...
namespace Sitrep.Host.Tests
{
    ...
    public class VesselViewProviderTests
    {
        ...
        public void BuildOrbitMapsThePatchChainWithBodyNameStringsAndTranslatesImpactToCollision()
        {
                ...
                orbit: new Dictionary<string, object?>
                {
                    ...
                    ["patches"] = new List<object?>
                    {
                        new Dictionary<string, object?>
                        {
                            ...
                            // KspHost.BuildOrbitPatchEntry translates KSP's own
                            // "IMPACT" enum spelling to "COLLISION" -- assert
                            // the RAW dict already carries the translated
                            // string (this test exercises the Host-side
                            // mapper, not KspHost itself, but pins the
                            // contract between them).
                            ["patchEndTransition"] = "COLLISION",
                            ["peA"] = 90_000.0,
                            ["apA"] = 240_000.0,
                            ["semiLatusRectum"] = 690_000.0,
                            ["semiMinorAxis"] = 695_000.0,
                            ["referenceBody"] = "Kerbin",
            ...
            Assert.NotNull(orbit);
            var patch = Assert.Single(orbit!.Patches);
            Assert.Equal(700_000.0, patch.Sma);
            Assert.Equal(0.01, patch.Ecc);
            Assert.Equal(2500.0, patch.Period);
            Assert.Equal(0.0, patch.StartUt);
            Assert.Equal(2500.0, patch.EndUt);
            Assert.Equal(TransitionType.Initial, patch.PatchStartTransition);
            Assert.Equal(TransitionType.Collision, patch.PatchEndTransition);
            Assert.Equal(90_000.0, patch.PeA);
            Assert.Equal(240_000.0, patch.ApA);
            Assert.Equal(690_000.0, patch.SemiLatusRectum);
            Assert.Equal(695_000.0, patch.SemiMinorAxis);
            Assert.Equal("Kerbin", patch.ReferenceBody);
            Assert.Null(patch.ClosestEncounterBody);
        }

        [Fact]
        public void BuildOrbitDefaultsPatchesToAnEmptyListNeverNull()
        {
            var snapshot = SnapshotWith(
                identity: new Dictionary<string, object?> { ["id"] = VesselGuid },
                orbit: new Dictionary<string, object?>
        ...
        public void BuildAttitudeMapsBothNamedFrames()
        {
            ...
            Assert.Equal(11.0, attitude.PitchRootFrame);
            Assert.Equal(268.5, attitude.HeadingRootFrame);
            Assert.Equal(-2.5, attitude.RollRootFrame);
        ...
        [Fact]
        public void BuildAttitudeWireCarriesBothFrames()
        {
            var snapshot = SnapshotWith(
                identity: new Dictionary<string, object?> { ["id"] = VesselGuid },
                attitude: new Dictionary<string, object?>
            ...
            var wire = VesselViewProvider.BuildAttitudeWire(snapshot);
            var wireDict = Assert.IsType<Dictionary<string, object?>>(wire);

            Assert.Equal(12.5, wireDict["pitch"]);
            Assert.Equal(270.0, wireDict["heading"]);
            Assert.Equal(-3.0, wireDict["roll"]);
            Assert.Equal(11.0, wireDict["pitchRootFrame"]);
            Assert.Equal(268.5, wireDict["headingRootFrame"]);
            Assert.Equal(-2.5, wireDict["rollRootFrame"]);
        }
        ...
        [Fact]
        public void BuildThermalMapsHottestEngineAndOverheatFlag()
        {
            var snapshot = SnapshotWith(
                identity: new Dictionary<string, object?> { ["id"] = VesselGuid },
                thermal: new Dictionary<string, object?>
                {
                    ["maxInternalTempRatio"] = 0.95,
                    ["hottestEngineTemp"] = 1850.0,
                    ["hottestEngineMaxTemp"] = 2000.0,
                    ["hottestEngineTempRatio"] = 0.925,
            ...
            Assert.NotNull(thermal);
            Assert.Equal(1850.0, thermal!.HottestEngineTemp);
            Assert.Equal(2000.0, thermal.HottestEngineMaxTemp);
            Assert.Equal(0.925, thermal.HottestEngineTempRatio);
            Assert.True(thermal.AnyEnginesOverheating);
        }

        [Fact]
        public void BuildThermalLeavesEngineFieldsNullWhenVesselHasNoEngineParts()
        {
            var snapshot = SnapshotWith(
                identity: new Dictionary<string, object?> { ["id"] = VesselGuid },
                thermal: new Dictionary<string, object?>
            ...
            Assert.NotNull(thermal);
            Assert.Null(thermal!.HottestEngineTemp);
            Assert.Null(thermal.HottestEngineMaxTemp);
            Assert.Null(thermal.HottestEngineTempRatio);
            Assert.Null(thermal.AnyEnginesOverheating);
        }
        ...
        [Fact]
        public void BuildManeuverMapsEachNodesPostBurnPatchChain()
        {
            var snapshot = SnapshotWith(
                identity: new Dictionary<string, object?> { ["id"] = VesselGuid },
                maneuverNodes: new List<object?>
                    ...
                    new Dictionary<string, object?>
                    {
                        ...
                        ["patches"] = new List<object?>
                        {
                            new Dictionary<string, object?>
                            {
                                ...
                                ["startUt"] = 12345.0,
                                ["endUt"] = 15345.0,
                                // The FIRST post-burn patch starts with a
                                // MANEUVER transition -- BuildOrbitPatchChain
                                // walks from node.nextPatch, not the vessel's
                                // pre-burn orbit.
            ...
            Assert.NotNull(maneuver);
            var node = Assert.Single(maneuver!.Nodes);
            var patch = Assert.Single(node.Patches);
            Assert.Equal(800_000.0, patch.Sma);
            Assert.Equal(TransitionType.Maneuver, patch.PatchStartTransition);
            Assert.Equal(TransitionType.Final, patch.PatchEndTransition);
            Assert.Equal("Kerbin", patch.ReferenceBody);
        }
