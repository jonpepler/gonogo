...
/**
 * Bridge the sitrep-sdk facade's fail-loud shims to the SAME real
 * core/sitrep-client singletons this widget actually calls: mirrors
 ...
 * installed" the moment a test renders it, since the sdk shims resolve
 * via `globalThis.__GONOGO_SDK__`, not a bundled copy. Partial by design:
 * only wire members code under test actually calls (installTestHost's
 * own contract, see GonogoScansatUplink/GonogoKosUplink's identical
 * setups).
 */
installTestHost({
  defineUplinkClient,
