...
function computeStormEntity(
  ...
  return {
    ...
    // CommNet graph and point markers (SYSTEM_ENTITY_DEFAULT_LAYER's blob=1,
    // below connection-line=2/point=3), never a bright alarm competing with
    // them. `stormState === 2` shifts the hue toward the shared warn token
    // rather than raising emphasis, so "arrived" reads as distinct without
    // the marker ever cutting off what's drawn on top of it.
    style: {
      emphasis: "faint",
