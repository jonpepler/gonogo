...
describe("landing scalars: vessel.state.landing* (land.timeToImpact/speedAtImpact/bestSpeedAtImpact/suicideBurnCountdown)", () => {
  ...
  it("derives a predicted impact point when the patch chain actually crosses the surface", () => {
    const s = deriveVesselState(
      landingGet(
        {},
        { orbit: { ...LANDING_ORBIT, patches: [syntheticPatch()] } },
      ),
      0,
    );
    expect(s?.landingPredictedLat).not.toBeNull();
    expect(s?.landingPredictedLon).not.toBeNull();
    expect(Number.isFinite(s?.landingPredictedLat)).toBe(true);
    expect(Number.isFinite(s?.landingPredictedLon)).toBe(true);
  });

  it("is null when vessel.orbit carries no patches yet", () => {
    const s = deriveVesselState(landingGet({}, { orbit: LANDING_ORBIT }), 0);
    expect(s?.landingPredictedLat).toBeNull();
  ...
  it("is null when not descending (no landingTimeToImpact to bound the horizon)", () => {
    const s = deriveVesselState(
      landingGet(
        { verticalSpeed: 10 },
        { orbit: { ...LANDING_ORBIT, patches: [syntheticPatch()] } },
...
describe("vessel.state.orbitPatches: legacy-shaped patch chain (o.orbitPatches)", () => {
  ...
  it("is an empty array (not undefined/null) when vessel.orbit carries no patches", () => {
    const s = deriveVesselState(
      patchTestGet(Quality.OnRails, LANDING_ORBIT_FOR_PATCH_TEST),
      0,
    );
    expect(s?.orbitPatches).toEqual([]);
  });
});
...
function syntheticPatchFixture(): WirePatch {
  return {
    ...
    startUt: 5,
    endUt: 105,
    patchStartTransition: 0, // INITIAL
    patchEndTransition: 1, // FINAL
    peA: 100_000,
    apA: 400_000,
    semiLatusRectum: 160_000,
