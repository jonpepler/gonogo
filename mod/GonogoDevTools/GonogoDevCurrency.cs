...
namespace Gonogo.DevTools
{
    ...
    public sealed class GonogoDevCurrency : MonoBehaviour
    {
        ...
        {
            public DelaySubsystem(bool present, bool scenarioLive, int scenarioInstances, bool subscribed, int pendingRows, double shadowScience, string firstPending, string fault)
            {
                Present = present;
            ...
            public static DelaySubsystem Absent(string fault) =>
                new DelaySubsystem(false, false, 0, false, 0, 0.0, "", fault);
        }
        ...
        private static DelaySubsystem ReadDelaySubsystem()
        {
            try
            {
                ...
                if (scenario == null)
                {
                    return new DelaySubsystem(true, false, 0, false, 0, 0.0, "", "scenario type present but no live instance");
                }
                ...
                return new DelaySubsystem(true, true, instances, subscribed, pendingRows, shadowScience, firstPending, fault);
            }
            catch (Exception ex)
            {
