...
namespace Sitrep.Host.Tests
{
    ...
    public class StageDeltaVViewProviderTests
    {
        ...
        public void BuildStagesMapsStageScopedResourcesWhenPresent()
        {
                ...
                stages: new List<object?>
                {
                    new Dictionary<string, object?>
                    {
                        ...
                        ["resources"] = new Dictionary<string, object?>
                        {
                            ...
                            ["Oxidizer"] = new Dictionary<string, object?> { ["current"] = 980.0, ["max"] = 1220.0 },
                        },
                    },
                },
                summary: BasicSummary());
            ...
            var stages = Assert.IsType<List<object?>>(StageDeltaVViewProvider.BuildStages(snapshot));
            var first = Assert.IsType<Dictionary<string, object?>>(stages[0]);
            var resources = Assert.IsType<Dictionary<string, object?>>(first["resources"]);

            var liquidFuel = Assert.IsType<Dictionary<string, object?>>(resources["LiquidFuel"]);
            Assert.Equal(800.0, liquidFuel["current"]);
            Assert.Equal(1000.0, liquidFuel["max"]);
            Assert.Equal(true, liquidFuel["active"]);

            var oxidizer = Assert.IsType<Dictionary<string, object?>>(resources["Oxidizer"]);
            Assert.Equal(980.0, oxidizer["current"]);
            Assert.Equal(1220.0, oxidizer["max"]);
        }

        [Fact]
        public void BuildStagesMapsStageResourcesToAnEmptyDictWhenAbsent()
        {
            // A stage entry built before this field existed, or a stage with no
            // tracked resources at all -- an empty map, never null/missing.
            var snapshot = DeltaVSnapshot(
                stages: new List<object?>
                {
                    new Dictionary<string, object?> { ["stage"] = 0, ["dvVac"] = 3200.0 },
                },
                summary: BasicSummary());
