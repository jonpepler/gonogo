/**
 * KerbalismUplink client-owned Topic registration: the bare presence gate
 * and the five relocated structured payload Topics.
 *
 ...
 *
 * `kerbalism.spaceweather` / `.profile` / `.lifesupport` / `.crew` /
 * `.features` used to be generated straight into `@ksp-gonogo/sitrep-sdk`,
 * because their payload types lived in Sitrep.Contract. They moved into
 * THIS Uplink's own contract slice (GonogoKerbalismUplink.Contract), so they
 * are registered here too, the same two halves `kerbalism.available`
 * already used, just with real generated payload types behind them instead
 * of `boolean`:
 *
 *   - TYPE: a `declare module "@ksp-gonogo/sitrep-sdk"` augmentation adds
 *     each Topic to `TopicPayloadMap`, so `useTelemetry("kerbalism.crew")`
 *     resolves to `KerbalismCrewEntry[]` in any program that statically
 *     imports this module.
 *   - RUNTIME: `registerBarePrimitiveTopic(...)` at module load feeds the
 *     SDK's runtime registry, so `isTopicId` / `getAllKnownTopicIds`
 *     enumerate them without the SDK ever naming the string.
 *
 * `index.ts` imports this module for its side effect (the registrations
 * plus the ambient augmentation), so importing the package wires every half.
 */
import {
 ...
 *
 * `registerTopicUnits` covers a Topic's OWN fields. That is the whole job
 * for a flat payload, and none of these five is flat:
 *
 ...
 * shape, then recurses through `wrapTypePayload`, which resolves that shape
 * BY TYPE NAME via `unitsForType`/`shapesForType`. Those read the SDK's
 * TYPE-keyed generated maps, which no longer carry a single Kerbalism
 * entry. So without the `registerTypeUnits` loop below, every per-kerbal
 * rule dose, every per-star distance, every process capacity and every
 * greenhouse flux would arrive as a BARE NUMBER while
 * ../__generated__/contract.ts still types it Value<...>. `topics.test.ts`
 * pins exactly that, at decode time through a real TelemetryClient, and
 * asserts the two halves separately so neither can stand in for the other.
 *
 * KerbalismStarInfo.direction makes the type registry load-bearing one step
 * further than the topic registry could ever reach: it is a Vec3 whose
 * unit is declared on the FIELD and emitted as three dotted leaf keys
 * ("direction.x"/".y"/".z"), on a type that is only ever reached through
 * spaceweather's `stars` array. Two hops of shape resolution, then a
 * fan-out to three leaves.
 *
 * Driven by looping over the generated maps rather than naming each entry,
 * so a type or Topic added to this Uplink's contract later needs no new
 * call site.
 */
