    ...
    public class Rp1BuildCommandsTests : IDisposable
    {
        ...
        public void Declares_every_write_command_undelayed_and_gated()
        {
            ...
            Assert.All(declarations, declaration =>
            {
                ...
                // that is the only condition evaluable before the press for five
                // of the six. Starting a build from a craft file declares a second,
                // because it also needs an install that can OPEN a craft file, and
                // that is core's rather than RP-1's.
                Assert.Contains(
