 ...
 * `TelemetryClient`/`TimelineStore` pipeline fed by a `StubTransport`, so a
 * test drives the actual streaming path rather than a mock registry.
 *
 * Copied rather than imported because the existing copies live in Uplink
 * packages, which the app must not depend on (that is the direction the
 * uplink-boundary guard exists to protect). If a fourth copy appears, promote
 * it into `@ksp-gonogo/sitrep-client` instead of adding one.
 *
